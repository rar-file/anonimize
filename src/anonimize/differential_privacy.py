...
class LaplaceMechanism(Mechanism):
    ...
    def confidence_interval(self, value: float, confidence: float = 0.95) -> Tuple[float, float]:
...
class GaussianMechanism(Mechanism):
    ...
    def _inverse_erf(self, x: float) -> float:
        ...
        y = math.log(1 - x * x)
        z = 2 / (math.pi * a) + y / 2
        return math.copysign(1.0, x) * math.sqrt(
            math.sqrt(z * z - y / a) - z
        )
...
class PrivacyBudgetTracker:
    ...
    def __init__(
        ...
        logger.debug(f"PrivacyBudgetTracker initialized: ε={total_epsilon}, δ={total_delta}")
    ...
    def get_usage_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "delta_percentage_used": (self.used_delta / self.total_delta) * 100 if self.total_delta > 0 else 0,
            "query_count": len(self._query_history),
        }
    
    def reset(self) -> None:
        """Reset the privacy budget (use with caution - breaks privacy guarantees)."""
        logger.warning("Privacy budget reset - this breaks differential privacy guarantees")
        self.used_epsilon = 0.0
        self.used_delta = 0.0
...
class DPAnonymizer:
    ...
    def __init__(
        ...
        logger.info(f"DPAnonymizer initialized with {mechanism} mechanism, "
    ...
    def anonymize_numeric(
        ...
        delta = 0.0 if self.mechanism_type == "laplace" else self.budget.total_delta / 10
    ...
    def anonymize_sum(
        ...
        return self.anonymize_numeric(sum_value, sensitivity=sensitivity, epsilon=epsilon)

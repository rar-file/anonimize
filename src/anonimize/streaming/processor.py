...
from anonimize.connectors import BaseConnector, ConnectionConfig
...
class StreamingProcessor:
    ...
    def process_file(
        ...
        if resume and self._checkpoint_path and self._checkpoint_path.exists():
            ...
            logger.info(f"Resuming from checkpoint: {rows_to_skip} rows already processed")
        ...
        with output_handler.write_streaming(output_path) as writer:
            ...
            for batch_num, batch in enumerate(batch_iterator, 1):
                ...
                # Save checkpoint
                if (batch_num % self.config.checkpoint_interval == 0 and 
    ...
    def process_database(
        ...
        logger.info(f"Starting database streaming process: {table_name} -> {output_path}")

...
class CSVAnonymizer(BaseAnonymizer):
    ...
    def anonymize(
        ...
        **kwargs,
        ...
        with open(input_path, encoding=self.encoding, newline="") as f:
            sample = f.read(8192)
            f.seek(0)
        ...
        with open(input_path, encoding=self.encoding, newline="") as infile, open(
                ...
                if column_mapping:
                    for csv_col, config_key in column_mapping.items():
                        if csv_col in row_dict and config_key in config:
                            ...
                            row_dict = self._core_anonymizer.anonymize(
                                row_dict, temp_config
                            )
                            fields_anonymized += 1
                else:

...
def anonymize(
    ...
    **kwargs
    ...
    if suffix == ".csv":
        return _anonymize_csv(input_path, output_path, strategy, dry_run, progress, **kwargs)
    elif suffix in (".json", ".jsonl"):
        return _anonymize_json(input_path, output_path, strategy, dry_run, progress, **kwargs)
    else:
        raise ValueError(
...
def detect_pii(data: Union[Dict[str, Any], List[Dict[str, Any]], str, Path]) -> Dict[str, Any]:
...
def _show_progress(input_path: Path, csv_anon: CSVAnonymizer, output_path: Path, config: Dict):

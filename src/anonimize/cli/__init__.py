...
from anonimize import __version__, anonymize, anonymize_data, detect_pii, preview
    ...
    try:
        ...
        if args.format == "json":
            ...
            for col, info in detected.items():
                pii_type = (
                    info.get("type", "unknown") if isinstance(info, dict) else info
                )
                confidence = (
...
def cmd_preview_cmd(args) -> int:
    ...
    try:
        preview_data = preview(
            file_path, num_rows=args.num_rows, strategy=args.strategy
        )
        ...
        header_row = " | ".join(
            h.ljust(col_widths[h]) for h in headers[:6]
        )  # Limit cols
        print(f"  {header_row}")
        ...
        for row in preview_data:
            values = [
                str(row.get(h, ""))[: col_widths[h] - 2].ljust(col_widths[h])
                for h in headers[:6]
            ]
            print(f"  {' | '.join(values)}")
...
"""
        ...
        print_info(
            "Edit this file and use with: anonimize data.csv --config "
            + str(output_path)
        )
        return 0
        ...
        if args.dry_run:
            ...
            if isinstance(result, dict):
                if "would_anonymize" in result:
                    ...
                    if cols:
                        print(
                            f"Would anonymize {len(cols)} column(s): {', '.join(cols)}"
                        )
                    else:
...
Report issues: https://github.com/rar-file/anonimize/issues
    ...
    anonymize_parser.add_argument(
        "file", help="Input file to anonymize (CSV, JSON, JSONL)"
    )
    anonymize_parser.add_argument(
        "-o", "--output", help="Output file path (default: input.anonymized.ext)"
    )
    anonymize_parser.add_argument(
        "-s",
        "--strategy",
    ...
    detect_parser = subparsers.add_parser(
        "detect", help="Detect PII in a file without anonymizing"
    )
    ...
    preview_parser = subparsers.add_parser(
        "preview", help="Preview anonymization on first few rows"
    )
    ...
    config_parser.add_argument(
        "--generate", action="store_true", help="Generate a sample config file"
    )
    config_parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "-w", "--wizard", action="store_true", help="Launch interactive wizard"
    )
